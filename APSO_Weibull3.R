...
##cdfweibull3
cdfweibull=function(x,par) {
  result <- ifelse(x <= par[3],0,1 - exp(-((x - par[3]) / par[2])^par[1]))
  return(result)
}
#thres-est
bmle_f <- function(dat){
...
#MLE for weibull2(two step)
twostepweibull=function(dat){
  par3=bmle_f(dat)
  if (par3==Inf) {par3=dat[1]-1e-5}
  n0=length(dat)
  nllike_w2=function(x){
    n0*x[1]*log(x[2])-n0*log(x[1])-(x[1]-1)*sum(log(dat-par3))+sum((dat-par3)^x[1])/(x[2]^x[1])}
  par12=optim(par=c(1,1),fn=nllike_w2)$par
  return(c(par12,par3))
}
#1993 MPS
MPS=function(dat,lower,upper,w=c(1.1,0.1),c0=c(1.49,1.49),s=10){
  n0=length(dat)+1
  MPSL=function(x){
    y=1-exp(-((dat-x[3])/x[2])^x[1])
    D=c(y[1],diff(y),1-y[n0-1])
    -sum(log(D))/n0
  }
  MPSE=psoptim(par=rep(NA,3),fn=MPSL,lower=lower,upper=upper,control=list(w=w,c.p=c0[1],c.g=c0[2],s=s))$par
  return(MPSE)
}
#2005 BL
BL=function(dat,lower,upper,w=c(1.1,0.1),c0=c(1.49,1.49),s=10){
  n0=length(dat)
  nllike_w3=function(x){
    n0*x[1]*log(x[2])-n0*log(x[1])-(x[1]-1)*sum(log(dat-x[3]))+
      sum((dat-x[3])^x[1])/(x[2]^x[1])-log(dat[1]-x[3])+log(dat[2]-x[3])}
  BLE=psoptim(par=rep(NA,3),fn=nllike_w3,lower=lower,upper=upper,control=list(w=w,c.p=c0[1],c.g=c0[2],s=s))$par
  return(BLE)
}
BL2=function(dat,par){
  n0=length(dat)
  nllike_w3=function(x){
    n0*x[1]*log(x[2])-n0*log(x[1])-(x[1]-1)*sum(log(dat-x[3]))+sum((dat-x[3])^x[1])/(x[2]^x[1])-log(dat[1]-x[3])+log(dat[2]-x[3])}
  MLE=optim(par=par,fn=nllike_w3)$par
  return(MLE)
}
#MLE
ML=function(dat,lower,upper,w=c(1.1,0.1),c0=c(1.49,1.49),s=10){
  n0=length(dat)
  nllike_w3=function(x){
    n0*x[1]*log(x[2])-n0*log(x[1])-(x[1]-1)*sum(log(dat-x[3]))+sum((dat-x[3])^x[1])/(x[2]^x[1])}
  MLE=psoptim(par=rep(NA,3),fn=nllike_w3,lower=lower,upper=upper,control=list(w=w,c.p=c0[1],c.g=c0[2],s=s))$par
  return(MLE)
}
ML2=function(dat,par){
  n0=length(dat)
  nllike_w3=function(x){
    n0*x[1]*log(x[2])-n0*log(x[1])-(x[1]-1)*sum(log(dat-x[3]))+sum((dat-x[3])^x[1])/(x[2]^x[1])}
  MLE=optim(par=par,fn=nllike_w3)$par
...
clusterExport(cl, c("cdfweibull","bmle_f","twostepweibull","BL","ML","MPS","ite","n","shapev","lenshape","reliability0","lenrelia"))
results_list <- foreach(shapei=shapev,.packages = c("FAdist","rootSolve","pso","fitdistrplus")) %do% {
  #pars
  ...
  trues=c(shapei,1,1)
  set.seed(20250712)
  datm=matrix(rweibull3(n*ite,trues[1],trues[2],trues[3]),n)
  bootn=500;QL0=qweibull3(1-reliability0,shape=trues[1],scale=trues[2],thres=trues[3])
  shaperange=matrix(0,nrow=ite,ncol=2);scalerange=matrix(0,nrow=ite,ncol=2);locationrange=matrix(0,nrow=ite,ncol=2)
  for (i in 1:ite){
    print(c(shapei,i))
    dat=sort(datm[,i])
    par0=twostepweibull(dat);parple[,i]=par0
    for (relia in 1:lenrelia){
      qlple[relia,i]=par0[3]+par0[2]*(-log(reliability0[relia]))^(1/(par0[1]))}
    reliaple[,i]=1-cdfweibull(QL0,par0)
    lifemple[i]=par0[3]+par0[2]*gamma(1+1/(par0[1]))
    
    partwostep=matrix(0,nrow=3,ncol=bootn)
    for (B in 1:bootn) {
      datboot=sort(rweibull3(n,shape=par0[1],scale=par0[2],thres=par0[3]))
      partwostep[,B]=twostepweibull(datboot)}
    ...
    upper1=c(shaperange[i,][2],scalerange[i,][2],locationrange[i,][2]);upper2=c(20,10,dat[1]-1e-10)
    
    parmle[,i]=tryCatch({ML(dat,lower=lower1,upper=upper1)},error=function(e) {rep(1e-5,3)})
    for (relia in 1:lenrelia){
      qlmle[relia,i]=parmle[,i][3]+parmle[,i][2]*(-log(reliability0[relia]))^(1/(parmle[,i][1]))}
    reliamle[,i]=1-cdfweibull(QL0,parmle[,i])
    lifemmle[i]=parmle[,i][3]+parmle[,i][2]*gamma(1+1/(parmle[,i][1]))
    
    parmle2[,i]=tryCatch({ML(dat,lower=lower2,upper=upper2)},error=function(e) {rep(1e-5,3)})
    for (relia in 1:lenrelia){
      qlmle2[relia,i]=parmle2[,i][3]+parmle2[,i][2]*(-log(reliability0[relia]))^(1/(parmle2[,i][1]))}
    reliamle2[,i]=1-cdfweibull(QL0,parmle2[,i])
    lifemmle2[i]=parmle2[,i][3]+parmle2[,i][2]*gamma(1+1/(parmle2[,i][1]))
    
    parbl[,i]=tryCatch({BL(dat,lower=lower1,upper=upper1)},error=function(e) {rep(1e-5,3)})
    for (relia in 1:lenrelia){
      qlbl[relia,i]=parbl[,i][3]+parbl[,i][2]*(-log(reliability0[relia]))^(1/(parbl[,i][1]))}
    reliabl[,i]=1-cdfweibull(QL0,parbl[,i])
    lifembl[i]=parbl[,i][3]+parbl[,i][2]*gamma(1+1/(parbl[,i][1]))
    
    parbl2[,i]=tryCatch({BL(dat,lower=lower2,upper=upper2)},error=function(e) {rep(1e-5,3)})
    for (relia in 1:lenrelia){
      qlbl2[relia,i]=parbl2[,i][3]+parbl2[,i][2]*(-log(reliability0[relia]))^(1/(parbl2[,i][1]))}
    reliabl2[,i]=1-cdfweibull(QL0,parbl2[,i])
    lifembl2[i]=parbl2[,i][3]+parbl2[,i][2]*gamma(1+1/(parbl2[,i][1]))
    
    parmps[,i]=tryCatch({MPS(dat,lower=lower1,upper=upper1)},error=function(e) {rep(1e-5,3)})
    for (relia in 1:lenrelia){
      qlmps[relia,i]=parmps[,i][3]+parmps[,i][2]*(-log(reliability0[relia]))^(1/(parmps[,i][1]))}
    reliamps[,i]=1-cdfweibull(QL0,parmps[,i])
    lifemmps[i]=parmps[,i][3]+parmps[,i][2]*gamma(1+1/(parmps[,i][1]))
    
    parmps2[,i]=tryCatch({MPS(dat,lower=lower2,upper=upper2)},error=function(e) {rep(1e-5,3)})
    for (relia in 1:lenrelia){
      qlmps2[relia,i]=parmps2[,i][3]+parmps2[,i][2]*(-log(reliability0[relia]))^(1/(parmps2[,i][1]))}
    reliamps2[,i]=1-cdfweibull(QL0,parmps2[,i])
    lifemmps2[i]=parmps2[,i][3]+parmps2[,i][2]*gamma(1+1/(parmps2[,i][1]))
  }
...
for (shapei in 1:lenshape){
  ...
  QL0=qweibull3(1-reliability0,shape=shapev[shapei],scale=parstrue[2],thres=parstrue[3])
  lifem0=parstrue[3]+parstrue[2]*gamma(1+1/(parstrue[1]))
...
library(openxlsx)
write.xlsx(result_df_merge, file = "n_20_four methods.csv", rowNames = FALSE)
save.image(file="n_20_four methods.RData")
